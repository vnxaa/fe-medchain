...
const StaffList = () => {
  ...
  const fetchStaffs = async () => {
    try {
      const response = await axios.get(`${process.env.service}/api/staff/`);
  ...
  const totalPages = Math.ceil(staffs.length / accountsPerPage);

  // Get current accounts based on pagination
  const indexOfLastAccount = currentPage * accountsPerPage;
  const indexOfFirstAccount = indexOfLastAccount - accountsPerPage;
  const currentAccounts = filteredStaffs.slice(
    indexOfFirstAccount,
    indexOfLastAccount
  );
  // Update current page
      ...
      <div className="sm:container sm:mx-auto">
        ...
        <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
          <div className="flex p-4 items-center justify-between py-4 bg-white dark:bg-gray-800">
            <div className="font-medium">Danh sách nhân viên</div>
