  ...
  const handleLogin = async () => {
    try {
      ...
      const response = await axios.post(
        `${process.env.service}/api/auth/doctor/login`,
        {
      ...
      if (response.data.status == "Doctor account is pending approval") {
        router.push("/Doctor/Permission/Wait");
      } else if (response.data.status == "Doctor account is rejected") {
        router.push("/Doctor/Permission/Reject");
      } else {
      ...
      // Replace "/hospital" with your desired page
    } catch (error) {
      // Handle any errors
      console.error(error);
    }
      ...
      <section className="bg-gray-50 dark:bg-gray-900">
        <div className="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
          ...
          <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
            <div className="flex flex-col items-center p-6 content-center space-y-4 md:space-y-6 sm:p-8">
              <h1 className="text-xl font-bold text-center leading-tight tracking-tight text-gray-900 md:text-2xl dark:text-white">
                Đăng nhập vào tài khoản bác sĩ
              </h1>
